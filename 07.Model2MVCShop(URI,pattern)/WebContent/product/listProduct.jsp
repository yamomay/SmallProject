...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<td colspan="6" class="ct_list_b01">
			<c:if test="${search.order!='reg_date'}">
				<a href="/product/listProduct?order=reg_date&menu=${menu}">신상품순</a>
			</c:if>
			<c:if test="${search.order=='reg_date'}">
				<input type="hidden" name="order" value="reg_date"/>
				<a href="/product/listProduct?menu=${menu}"><b>신상품순</b></a>
			</c:if>
			
			 / 
			 
			<c:if test="${search.order!='price'}">
				<a href="/product/listProduct?order=price&menu=${menu}">가격순</a>
			</c:if>
			<c:if test="${search.order=='price'}">
				<input type="hidden" name="order" value="price"/>
				<a href="/product/listProduct?menu=${menu}"><b>가격순</b></a>
			</c:if>
			
			 / 
			 
			<c:if test="${search.order!='prod_name'}">
				<a href="/product/listProduct?order=prod_name&menu=${menu}">상품명순</a>
			</c:if>
			<c:if test="${search.order=='prod_name'}">
				<input type="hidden" name="order" value="prod_name"/>
				<a href="/product/listProduct?menu=${menu}"><b>상품명순</b></a>
			</c:if>
	...
	<tr class="ct_list_pop">
			...
			<c:if test="${ product.proTranCode.trim() > 0 }">
				<c:choose>
					...
					<c:when test="${ product.proTranCode.trim() == 1 && user.role == 'admin'}">
						구매완료 <a href="/purchase/updateTranCodeByProd?currentPage=${resultPage.currentPage}&tranCode=2&prodNo=${product.prodNo}">배송하기</a>
						</td>
					</c:when>
		...
		<c:if test="${!empty user.userId}">
			<input type="button" style="height:15pt; text-align:center;" onclick="location.href='/product/likeProduct?prod_no=${product.prodNo}&menu=${menu}&currentPage=${resultPage.currentPage}'" value="♥" class="ct_list_like"/>
		</c:if>
