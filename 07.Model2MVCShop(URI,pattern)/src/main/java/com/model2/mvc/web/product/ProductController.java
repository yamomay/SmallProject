...
public class ProductController {
	...
	public String getProduct( @RequestParam("prodNo") int prodNo,@RequestParam("menu") String menu, HttpServletResponse response, HttpServletRequest request ) throws Exception {
		...
		System.out.println("쿠키생성여부 확인 : "+cookie);
	...
	public String likeProduct(HttpSession session,@RequestParam("menu") String menu,@RequestParam("prod_no") String prodNo,@RequestParam("currentPage") int currentPage) throws Exception {
		...
		if(isLike) {
			return "redirect:/product/listProduct?menu="+menu+"&currentPage="+currentPage+"&likefail=1";
		}else {
			productService.likeProduct(user.getUserId(),prodNo);
			return "redirect:/product/listProduct?menu="+menu+"&currentPage="+currentPage;
		}
	}
